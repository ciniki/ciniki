...
function file_put_min_cssjs($device, $engine, $size) {
    ...
    //
    if( $engine != 'trident' ) {
        $new_content = $all_browser_js . "\n" . ${"s_${size}_js"} . "\n" . ${"e_${engine}_js"};
        if( file_exists("$ciniki_root/site/ciniki-mods/core/ui/$device-$engine.min.js") ) {
            $old_content = file_get_contents("$ciniki_root/site/ciniki-mods/core/ui/$device-$engine.min.js");
            // 
            // Get the previous minified version and use for checking if any updates have been done
            //
            if( preg_match("/'version':'([0-9]{6}\.[0-9]{4})',/", $old_content, $matches) ) {
                $version = $matches[1];
                $new_content = preg_replace("/'version':'[0-9]{6}\.[0-9]{4}',/", "'version':'$version',", $new_content);
            }
        } else {
            $old_content = '';
        }
        if( $new_content != $old_content ) {
            print "Updating: $ciniki_root/site/ciniki-mods/core/ui/$device-$engine.min.js\n";
            //
            // Set a new version in the minified code
            //
            $version = strftime("%y%m%d.%H%M", time());
            $new_content = preg_replace("/'version':'[0-9]{6}\.[0-9]{4}',/", "'version':'$version',", $new_content);
            file_put_contents("$ciniki_root/site/ciniki-mods/core/ui/$device-$engine.min.js", $new_content);
        }
    }
    ...
    // Save css
    //
    $new_content = $style_css . ${"e_${engine}_css"} . ${"s_${size}_css"} . ${"d_${device}_css"} . ${"s_${size}_${engine}_css"};
    if( file_exists("$ciniki_root/site/ciniki-mods/core/ui/themes/default/$device-$engine.min.css") ) {
        $old_content = file_get_contents("$ciniki_root/site/ciniki-mods/core/ui/themes/default/$device-$engine.min.css");
    } else {
        $old_content = '';
    }
    if( $new_content != $old_content ) {
        print "Updating: $ciniki_root/site/ciniki-mods/core/ui/themes/default/$device-$engine.min.css\n";
        file_put_contents("$ciniki_root/site/ciniki-mods/core/ui/themes/default/$device-$engine.min.css", $new_content);
    }
}
